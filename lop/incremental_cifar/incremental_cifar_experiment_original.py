...
class IncrementalCIFARExperiment(Experiment):
    ...
    def __init__(self, exp_params: dict, results_dir: str, run_index: int, verbose=True):
        ...
        self.class_increase_frequency = access_dict(exp_params, key="task_epochs", default=200, val_type=int)
        self.num_epochs = (self.num_classes // 5) * self.class_increase_frequency
        ...
        self.checkpoint_save_frequency = 1  # save every epoch
    ...
    def extend_classes(self, training_data: CifarDataSet, test_data: CifarDataSet, val_data: CifarDataSet):
        ...
        if (self.current_epoch % self.class_increase_frequency) == 0:
            ...
            # Always use the best validation model for the next task
            if self.best_accuracy_model_parameters:
                self.net.load_state_dict(self.best_accuracy_model_parameters)
                self._print("\t\tLoaded best validation model for next task")
    ...
    def _save_model_parameters(self):
        ...
        file_name = "checkpoint_original_index-{0}_epoch-{1}.pt".format(self.run_index, self.current_epoch)
    ...
    def save_experiment_checkpoint(self):
        ...
        checkpoint_file_name = "index-{0}_original_{1}-{2}.p".format(self.run_index, self.checkpoint_identifier_name, self.current_epoch)
        ...
        if self.delete_old_checkpoints and self.current_epoch > self.checkpoint_save_frequency:
            old_checkpoint_file_name = "index-{0}_original_{1}-{2}.p".format(self.run_index, self.checkpoint_identifier_name, 
    ...
    def load_experiment_checkpoint(self):
        ...
        if os.path.exists(self.experiment_checkpoints_dir_path):
            checkpoint_files = [f for f in os.listdir(self.experiment_checkpoints_dir_path) if f.startswith(f"index-{self.run_index}_original")]
            if checkpoint_files:
                checkpoint_epochs = [int(f.split('-')[-1].split('.')[0]) for f in checkpoint_files]
                latest_epoch = max(checkpoint_epochs)
                checkpoint_file_name = f"index-{self.run_index}_original_{self.checkpoint_identifier_name}-{latest_epoch}.p"
                checkpoint_file_path = os.path.join(self.experiment_checkpoints_dir_path, checkpoint_file_name)
                self.load_checkpoint_data_and_update_experiment_variables(checkpoint_file_path)
...
def main():
    ...
    parser.add_argument("--task-epochs", action="store", type=int, default=200,
                        help="Number of epochs per task (class increase frequency).")
    parser.add_argument("--results-dir", action="store", type=str, default=None,
    ...
    if "results_dir" not in experiment_parameters.keys() or experiment_parameters["results_dir"] == "":
        if args.results_dir is not None:
            experiment_parameters["results_dir"] = args.results_dir
        else:
    ...
    # Add task epochs to experiment parameters
    experiment_parameters["task_epochs"] = args.task_epochs

    initial_time = time.perf_counter()
    # Use custom results directory directly if provided, otherwise use default structure
    if args.results_dir is not None:
        final_results_dir = experiment_parameters["results_dir"]
    else:
        final_results_dir = os.path.join(experiment_parameters["results_dir"], experiment_parameters["experiment_name"])
