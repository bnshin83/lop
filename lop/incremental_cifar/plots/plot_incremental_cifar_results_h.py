...
def line_plot_with_shaded_region(average, standard_error, color, label):
    """
    Creates a line plot with shaded regions and markers
    """
    # Convert task numbers to epochs (each task = 200 epochs, starting from epoch 200)
    x_epochs = (np.arange(average.size) + 1) * 200
    
    # Plot with markers and lines
    plt.errorbar(x_epochs, average, yerr=standard_error, 
                color=color, label=label, marker='o', markersize=6, 
                linewidth=2, capsize=4, capthick=1.5)
    
    # Add shaded region for error bounds
    plt.fill_between(x_epochs, average - standard_error, average + standard_error, 
                    color=lighten_color(color, 0.1), alpha=0.3)
...
def retrieve_results(algorithms: list, metric: str, results_dir: str):
    ...
    for alg in algorithms:
        ...
        if not os.path.exists(temp_dir):
            print(f"Warning: Directory not found: {temp_dir}")
            continue
            
        files = [f for f in os.listdir(temp_dir) if f.endswith('.npy')]
        if not files:
            print(f"Warning: No .npy files found in {temp_dir}")
            continue
            
        num_samples = len(files)
        
        # Load the first file to determine shape
        sample_file = os.path.join(temp_dir, files[0])
        sample_data = np.load(sample_file)
        
        # For next_task_curvature_analysis, we shouldn't bin the data
        if "curvature" in metric:
            # For curvature, just use the data as is
            temp_results = np.zeros((num_samples, len(sample_data)), dtype=np.float32)
            ...
            results_dict[alg] = temp_results
            
        else:
            # For other metrics, use the original binning approach
            temp_results = np.zeros((num_samples, total_num_epochs // epochs_per_task), dtype=np.float32)

            for index in range(num_samples):
                temp_result_path = os.path.join(temp_dir, f"index-{index}.npy")
            ...
            # This is the proper slicing - we want columns to start from start_idx
...
#         if start_idx >= temp_results.shape[1]:
#             start_idx = 0
...
def main():
    ...
    parser.add_argument("--metric", action="store", type=str, default="test_accuracy_per_epoch",
                        ...
                        choices=["next_task_dormant_units_analysis", "relative_accuracy_per_epoch",
                                 ...
                                 "test_loss_per_epoch", "weight_magnitude_analysis", "next_task_curvature_analysis"])
